...
export default function CountdownTimer({ endAt, className = "", tone = "light", showSeconds = false, size = "md", boxClassName = "", separatorClassName = "" }: CountdownTimerProps) {
	...
	const boxClasses = tone === "light" ? "bg-white text-black shadow-sm" : "bg-surface-secondary text-foreground border border-default";
	const separatorToneClasses = tone === "light" ? "text-white" : "text-foreground";
	const sizeClasses = size === "sm" ? { box: "h-7 min-w-7 px-1.5 text-sm", separator: "text-sm" } : size === "lg" ? { box: "h-10 min-w-10 px-2.5 text-lg", separator: "text-lg" } : { box: "h-8 min-w-8 px-2 text-base", separator: "text-base" };
			...
			{parts.map((part, partIndex) => (
				<div key={`${part}-${partIndex}`} className='flex items-center gap-1.5'>
					{part.split("").map((digit, digitIndex) => (
						<span key={`${digit}-${digitIndex}`} className={`inline-flex items-center justify-center rounded-md font-semibold tabular-nums ${boxClasses} ${sizeClasses.box} ${boxClassName}`}>
							{digit}
						</span>
					))}
