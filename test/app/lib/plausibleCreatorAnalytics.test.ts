...
describe("plausibleCreatorAnalytics", () => {
	...
	it("returns safe zeroes for missing or invalid metrics", () => {
		expect(parsePlausibleCreatorMetrics({ results: [{ metrics: [null, "invalid"] }] })).toEqual({ visitors: 0, pageviews: 0, visits: 0, bounceRate: 0, timeOnPage: 0, scrollDepth: 0 });
	});
	...
	it("uses the metric names stored in legacy Plausible payloads", () => {
		...
		).toEqual({ visitors: 12, pageviews: 34, visits: 10, bounceRate: 42.5, timeOnPage: 0, scrollDepth: 0 });
	});
	...
	it("parses timeseries and aggregates multi-dimensional breakdowns", () => {
		...
		expect(analytics.timeseries[0]).toEqual({ date: "2026-08-08", visitors: 4, pageviews: 8, visits: 3, bounceRate: 40, timeOnPage: 80, scrollDepth: 55 });
	...
	it("fills Plausible time labels that have no result row with zeroes", () => {
		...
		expect(analytics.timeseries).toEqual([
			{ date: "2026-08-07", visitors: 0, pageviews: 0, visits: 0, bounceRate: 0, timeOnPage: 0, scrollDepth: 0 },
			{ date: "2026-08-08", visitors: 4, pageviews: 8, visits: 3, bounceRate: 40, timeOnPage: 80, scrollDepth: 55 },
		]);
	});
	...
	it("creates a continuous zero-valued series when no analytics rows are available", () => {
		...
		expect(analytics.metrics).toEqual({ visitors: 0, pageviews: 0, visits: 0, bounceRate: 0, timeOnPage: 0, scrollDepth: 0 });
