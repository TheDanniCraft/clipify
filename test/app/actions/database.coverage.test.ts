...
describe("database.ts coverage tests", () => {
    ...
    it("covers applyPlaylistImportFilters logic", async () => {
        ...
        const clips = [
            { id: "c1", creator_name: "A", creator_id: "1", title: "T1", game_id: "G1", view_count: 100, created_at: "2020-01-01" },
            { id: "c2", creator_name: "B", creator_id: "2", title: "BAD", game_id: "G2", view_count: 50, created_at: "2020-01-02" },
        ];
        // Mock getPlaylistImportSourceClips or its dependencies
        // Actually, let's just mock getPlaylistClips and other stuff if needed
        // but it's easier to just mock the twitch call if it's called.
    });
    ...
    it("covers getSettings save error branch", async () => {
        ...
        // saveSettings calls:
        dbSelect.mockImplementationOnce(() => { throw new Error("DB Error"); }); // getAccessToken error
        await expect(getSettings("user-1")).rejects.toThrow("Failed to fetch settings");
    });
