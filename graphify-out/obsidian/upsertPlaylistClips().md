---
source_file: "src/app/actions/database.ts"
type: "code"
community: "Premium Gating & Chat Commands"
location: "L1166"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Premium_Gating__Chat_Commands
---

# upsertPlaylistClips()

## Connections
- [[database.ts]] - `contains` [EXTRACTED]
- [[getOwnerPlanContext()]] - `calls` [EXTRACTED]
- [[getPlaylistClipsForOwnerServer()]] - `calls` [EXTRACTED]
- [[importPlaylistClips()]] - `calls` [EXTRACTED]
- [[page.tsx_6]] - `imports` [EXTRACTED]
- [[page.tsx_8]] - `imports` [EXTRACTED]
- [[requirePlaylistAccess()]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Premium_Gating__Chat_Commands