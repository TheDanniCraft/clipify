...
RUN set -eux; \
    apt-get update; \
    apt-get install -y --no-install-recommends ca-certificates curl qemu-user-static; \
...
RUN --mount=type=cache,target=/root/.pkg-cache \
    ...
    RUNNER_PKG_TARGETS=node22-win-x64,node22-linux-x64,node22-linux-arm64,node22-macos-x64,node22-macos-arm64 \
