...
for (const key of ["--root", "--fingerprint", "--commit", "--repository", "--artifact-tag-prefix", "--oci-digests", "--output"]) {
...
for (const directory of readdirSync(root, { withFileTypes: true }).filter((entry) => entry.isDirectory())) {
	...
	artifacts.push({
		...
		oci: {
			reference: `${args.get("--repository")}:${args.get("--artifact-tag-prefix")}${args.get("--fingerprint")}-${metadata.platform}`,
			digest,
		},
	});
}
