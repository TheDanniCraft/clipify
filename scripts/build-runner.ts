...
async function main() {
	...
	const pkgScript = path.join(process.cwd(), "node_modules", "@yao-pkg", "pkg", "lib-es5", "bin.js");
	const pkgBin = process.platform === "win32" ? "node" : path.join(process.cwd(), "node_modules", ".bin", "pkg");
	...
	const pkgArgs = process.platform === "win32" ? [pkgScript, "build/runner.js", "-t", pkgTargets, "--out-path", "build/"] : ["build/runner.js", "-t", pkgTargets, "--out-path", "build/"];
